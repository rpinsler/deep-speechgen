...
# (linearly, exponentially, by a factor of 2-10), ADAM
...
def load(n=0, cut=0, use_delta=False, timesteps=100, shift=10):
    ...
    data, vuv = sp.split_samples(interp_data, vuv_data, timesteps, shift)
...
def build(timesteps, input_dim, M=None):
    if M is None:  # linear output layer
        ...
        model = Sequential()
        model.add(LSTM(512, input_shape=(timesteps, input_dim)))
        model.add(Dropout(0.2))
        model.add(Dense((input_dim-1+2)*M+1))
        # model.add(Dense((input_dim+2)*M))
        model.add(sp.GMMActivation(M))
        model.compile(loss=sp.gmm_loss, optimizer='rmsprop')
...
def predict(X_test, model, pred_len=0, M=None):
    ...
    for n in range(pred_len):
        if (n % 500) == 0:
            print("Predictions completed: " + str(n+1) + "/" + str(pred_len))
        ...
        signal = np.concatenate((signal[:, 1:, :],
                                 preds.reshape((1, 1, input_dim))), 1)
...
def postprocess(data, approach, pred_len, use_delta=False,
                X_train=None, mu=None, sigma=None):
    data[:, :-1] = sp.denormalize_data(data[:, :-1], mu[:-1], sigma[:-1])
...
def evaluate(pred, test_sample, y_test=None, show_plot=False, save_plot=None):
    if y_test is not None:
        # requires longer test trajectories
        mmcd, rmse, class_err = sp.evaluate(y_test, pred)

    plt.figure()
    plt.imshow(np.vstack((test_sample, pred)).T,
...
nb_epoch = 20
pred_len = 2000
cut = 100  # no. timesteps removed at beginning and end of each sample
M = 3  # no of GMM components
...
model = build(timesteps, input_dim, M)
...
if load_weights:
    ...
    with open('eval/hist_' + approach + '.p', 'wb') as handle:
        pickle.dump(hist.history, handle)

print("Predict")
pred, test_sample = predict(X_test, model, pred_len, M)
...
mu = sigma = None
pred = postprocess(pred, approach, pred_len, mu=mu, sigma=sigma)
