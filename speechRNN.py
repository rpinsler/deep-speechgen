...
def build(timesteps, input_dim):
    ...
    model = Sequential()
    model.add(
        LSTM(512, return_sequences=False, input_shape=(timesteps, input_dim)))
...
def predict(X_test, model, pred_len=0, M=None):
    ...
    for n in range(pred_len):
        if (n % 500) == 0:
            print("Predictions completed: " + str(n) + "/" + str(pred_len))
...
# y_train = preprocess(y_train, mu, sigma)
# X_test = preprocess(X_test, mu, sigma)
# y_test = preprocess(y_test, mu, sigma)
